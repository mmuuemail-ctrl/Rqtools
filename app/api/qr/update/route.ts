...
function isAllowedFile(filename: string, mimeType: string) {
  ...
  return (
    APP_CONFIG.allowedExtensions.includes(ext) &&
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const textContent = typeof formData.get("textContent") === "string"
    ...
    const maxViewsTotal = maxViewsEnabled
      ? maxViewsTotalThousands * 1000
      : null;
    ...
    const hasActivePaidSubscription =
      (profile.plan_type === "day" || profile.plan_type === "month" || profile.plan_type === "year") &&
    ...
    const currentQrRes = await supabaseAdmin
      .from("qr_codes")
      .select("file_key")
      .eq("id", qrId)
      .eq("user_id", userId)
    ...
    if (contentType === "media" && uploadedFile instanceof File) {
      ...
      const safeName = uploadedFile.name.replace(/[^\w.\- ]+/g, "").replace(/\s+/g, "-");
      const storageKey = `${userId}/${qrId}-${Date.now()}-${safeName}`;

      if (currentQrRes.data?.file_key) {
        await supabaseAdmin.storage.from("files").remove([currentQrRes.data.file_key]);
      }
      ...
      const publicData = supabaseAdmin.storage.from("files").getPublicUrl(storageKey);
